...
# Basic copy: each thread copies N elements using one vload + one vstore!
...
# Scale: load N, multiply each element by scale, store back
...
# Reduction: each thread sums its N elements and writes one scalar result
...
# Scalar tail: handles arrays whose length is not divisible by N
...
# Write-then-read: vstore! followed by vload from the same slot — tests store→load forwarding
function vload_store_load_kernel(arr, v0::T, v1::T, v2::T, v3::T) where {T}
    KI.vstore!(arr, 1, (v0, v1, v2, v3))
    loaded = KI.vload(Val(4), arr, 1)
    KI.vstore!(arr, 5, loaded)   # copy to second slot — must equal first
    return
end

# Multi-thread copy: workgroupsize > 1, each thread handles N consecutive elements
    ...
    @testset "KernelIntrinsics Tests" begin
        ...
        @testset "vload / vstore!" begin
            ...
            @testset "roundtrip N=$N T=$T" for N in (1, 2, 4, 8),
                ...
                n   = 64   # divisible by 8, covers all N values above
                src = AT(T.(1:n))
                dst = AT(zeros(T, n))
            ...
            # ── 3. Minimal array: size == N (one vload covers the whole array) ──
            ...
            @testset "per-thread sum N=4" begin
                n     = 64
            ...
            @testset "per-thread sum sequential N=4" begin
                # src = [1,2,...,64]: each group of 4 sums to 4k+10 for k=0..15
                n      = 64
                src    = AT(Float32.(1:n))
                result = AT(zeros(Float32, n ÷ 4))
                KI.@kernel backend() numworkgroups = n ÷ 4 workgroupsize = 1 vload_reduce_kernel(result, src, Val(4))
            ...
            @testset "vstore then vload identity" begin
                ...
                @test h[5:8] == h[1:4]   # loaded values == stored values
            end
            ...
            @testset "all-zeros roundtrip N=4" begin
                ...
                dst = AT(ones(Float32, n))   # pre-fill with non-zero to catch stale data
            ...
            # ── 10. Multi-thread (workgroupsize > 1): test concurrency ──────
            @testset "multi-thread workgroupsize=4 N=4" begin
                wgs = 4
                n   = 128   # 8 groups × 4 threads × 4 elements
                src = AT(Float32.(1:n))
                dst = AT(zeros(Float32, n))
            ...
            # ── 11. Integer types: no float-specific code paths ──────────────
            ...
            # ── 12. Type-stability: output element type matches input ─────────
